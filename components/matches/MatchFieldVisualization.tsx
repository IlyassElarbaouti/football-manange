    ...
    case "5-aside":
      return {
        ...
        positions: {
          ...
          desktop: {
            teamA: [
              { id: "GK", x: "7%", y: "50%", role: "Goalkeeper" },
              { id: "LB", x: "25%", y: "25%", role: "Left Back" },
              { id: "RB", x: "25%", y: "75%", role: "Right Back" },
              { id: "CM", x: "35%", y: "50%", role: "Midfielder" },
              { id: "ST", x: "47%", y: "50%", role: "Striker" },
            ],
            teamB: [
              { id: "GK", x: "93%", y: "50%", role: "Goalkeeper" },
              { id: "LB", x: "75%", y: "25%", role: "Left Back" },
              { id: "RB", x: "75%", y: "75%", role: "Right Back" },
              { id: "CM", x: "65%", y: "50%", role: "Midfielder" },
              { id: "ST", x: "53%", y: "50%", role: "Striker" },
            ],
          },
      ...
      return {
        ...
        positions: {
          desktop: {
            teamA: [
              { id: "GK", x: "7%", y: "50%", role: "Goalkeeper" },
              { id: "LB", x: "18%", y: "25%", role: "Left Back" },
              { id: "CB", x: "18%", y: "50%", role: "Center Back" },
              { id: "RB", x: "18%", y: "75%", role: "Right Back" },
              { id: "LM", x: "30%", y: "30%", role: "Left Midfielder" },
              { id: "RM", x: "30%", y: "70%", role: "Right Midfielder" },
              { id: "ST", x: "43%", y: "50%", role: "Striker" },
            ],
            teamB: [
              { id: "GK", x: "93%", y: "50%", role: "Goalkeeper" },
              { id: "LB", x: "82%", y: "25%", role: "Left Back" },
              { id: "CB", x: "82%", y: "50%", role: "Center Back" },
              { id: "RB", x: "82%", y: "75%", role: "Right Back" },
              { id: "LM", x: "70%", y: "30%", role: "Left Midfielder" },
              { id: "RM", x: "70%", y: "70%", role: "Right Midfielder" },
              { id: "ST", x: "57%", y: "50%", role: "Striker" },
            ],
          },
          mobile: {
            teamA: [
      ...
      return {
        ...
        positions: {
          desktop: {
            teamA: [
              { id: "GK", x: "7%", y: "50%", role: "Goalkeeper" },
              { id: "LB", x: "18%", y: "20%", role: "Left Back" },
              { id: "CB1", x: "18%", y: "40%", role: "Center Back" },
              { id: "CB2", x: "18%", y: "60%", role: "Center Back" },
              { id: "RB", x: "18%", y: "80%", role: "Right Back" },
              ...
              { id: "ST1", x: "43%", y: "35%", role: "Striker" },
              { id: "ST2", x: "43%", y: "65%", role: "Striker" },
            ],
            teamB: [
              { id: "GK", x: "93%", y: "50%", role: "Goalkeeper" },
              { id: "LB", x: "82%", y: "20%", role: "Left Back" },
              { id: "CB1", x: "82%", y: "40%", role: "Center Back" },
              { id: "CB2", x: "82%", y: "60%", role: "Center Back" },
              { id: "RB", x: "82%", y: "80%", role: "Right Back" },
              ...
              { id: "ST1", x: "57%", y: "35%", role: "Striker" },
              { id: "ST2", x: "57%", y: "65%", role: "Striker" },
            ],
          },
          mobile: {
            teamA: [
...
const getPositionCategory = (position) => {
  ...
  if (
    position.toLowerCase().includes("gk") ||
    position.toLowerCase() === "goalkeeper"
  ) {
    return "Goalkeeper";
  } else if (
    position.toLowerCase().includes("b") ||
    position.toLowerCase().includes("cb") ||
    position.toLowerCase().includes("back")
  ) {
    return "Defender";
  } else if (
    position.toLowerCase().includes("m") ||
    position.toLowerCase().includes("mid")
  ) {
    return "Midfielder";
  } else {
    return "Forward";
...
const MatchFieldVisualization = ({ match }) => {
  ...
  // Get field configuration based on match type using useMemo to prevent recalculations
  const fieldConfig = useMemo(
    () => getFieldConfig(match?.matchType || "11-aside"),
    [match?.matchType]
  );

  // Effect to detect screen size - only run this effect for screen size changes
  useEffect(() => {
  ...
  // Get positions based on screen size using useMemo to prevent unnecessary calculations
  const { teamAPositions, teamBPositions } = useMemo(() => {
    return {
      teamAPositions: isMobile
        ? fieldConfig.positions.mobile.teamA
        : fieldConfig.positions.desktop.teamA,
      teamBPositions: isMobile
        ? fieldConfig.positions.mobile.teamB
        : fieldConfig.positions.desktop.teamB,
    };
  }, [isMobile, fieldConfig]);

  // Calculate team balancing using useMemo to prevent recalculations on every render
  const { teamA, teamB, queue } = useMemo(() => {
    ...
    const sortedPlayers = [...allPlayers].sort(
      (a, b) => (b.user.skillLevel || 75) - (a.user.skillLevel || 75)
    );
    ...
    sortedPlayers.forEach((player, index) => {
      if (index % 2 === 0) {
        rawTeamA.push(player);
      } else {
        rawTeamB.push(player);
      }
    });
    ...
    const assignPlayers = (team, positions) => {
      ...
      positions.forEach((position) => {
        ...
        const matchingPlayer = team.find(
          (p) =>
            getPositionCategory(p.user.preferredPosition || "Any") ===
              positionCategory &&
        ...
        // Otherwise check if we have any player left
          ...
          const anyPlayer = team.find(
            (p) => !result.some((r) => r.user?._id === p.user._id)
          );
          if (anyPlayer) {
            result.push({
              ...anyPlayer,
    ...
    const assignedTeamA = assignPlayers(rawTeamA, teamAPositions);
    const assignedTeamB = assignPlayers(rawTeamB, teamBPositions);
    ...
    const queuePlayers = allPlayers.filter(
      (p) => !usedPlayerIds.has(p.user?._id)
    );

    // Add placeholders for total slots that haven't been filled
    const totalAssignedPlayers =
      assignedTeamA.filter((p) => !p.isEmpty).length +
    ...
    const placeholders = Array.from({
      length: Math.max(0, totalPlaceholdersNeeded),
    }).map((_, i) => ({
      isPlaceholder: true,
      position: "ANY",
      user: {
        name: `Player ${queuePlayers.length + i + 1}`,
      },
    }));
    ...
    return {
      teamA: assignedTeamA,
      teamB: assignedTeamB,
      queue: [...queuePlayers, ...placeholders],
    };
  }, [match?.players, teamAPositions, teamBPositions, match?.totalSlots]);
  ...
  return (
    <div className="space-y-6 mt-8">
      ...
      <div
        className={`relative ${fieldConfig.fieldClass} rounded-lg overflow-hidden border border-amber-500/20 bg-gradient-to-r md:bg-gradient-to-r bg-gradient-to-b from-green-800 to-green-700`}
      >
        {/* Field markings - responsive for mobile/desktop */}
        <div className="absolute inset-0 ">
          {/* Center circle */}
          <div
            className={`absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 ${fieldConfig.centerCircleSize} border-2 border-white/60 rounded-full`}
          ></div>

   {/* Center line - horizontal on mobile, vertical on desktop */}
   <div className="absolute md:top-0 md:left-1/2 md:w-0.5 md:h-full top-1/2 left-0 transform -translate-y-1/2 w-full h-0.5 bg-white/60"></div>
          ...
          <div className="md:absolute md:left-0 md:top-1/2 md:transform md:-translate-y-1/2 md:h-1/3 md:w-8 absolute top-0 left-1/2 transform -translate-x-1/2 w-32 h-8 border-2 border-white/60 md:border-l-0 border-t-0"></div>
          <div className="md:absolute md:right-0 md:top-1/2 md:transform md:-translate-y-1/2 md:h-1/3 md:w-8 absolute bottom-0 left-1/2 transform -translate-x-1/2 w-32 h-8 border-2 border-white/60 md:border-r-0 border-b-0"></div>
          
          {/* Penalty areas - adjust for mobile/desktop */}
          <div className="md:absolute md:left-0 md:top-1/2 md:transform md:-translate-y-1/2 md:h-1/2 md:w-16 absolute top-0 left-1/2 transform -translate-x-1/2 w-48 h-16 border-2 border-white/60 md:border-l-0 border-t-0"></div>
          <div className="md:absolute md:right-0 md:top-1/2 md:transform md:-translate-y-1/2 md:h-1/2 md:w-16 absolute bottom-0 left-1/2 transform -translate-x-1/2 w-48 h-16 border-2 border-white/60 md:border-r-0 border-b-0"></div>
           </div>
              ...
              {!player.isEmpty && (
                <TooltipContent>
                  <div className="space-y-1 p-1">
                    ...
                    <p className="text-xs">Position: {player.role}</p>
                    <p className="text-xs">
                      Skill: {player.user.skillLevel || 75}/100
                    </p>
                  </div>
                </TooltipContent>
              )}
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap gap-1">
              {teamA.map((player, index) => (
                <div
                  key={index}
                  className={`w-6 h-6 rounded-full ${player.isEmpty ? "md:bg-black/30 md:border-dashed md:border-amber-500/30 bg-white/30 border-dashed border-amber-500/30" : "md:bg-black md:border-amber-500/30 bg-white border-amber-500/30"} flex items-center justify-center border`}
                >
                  {!player.isEmpty && (
                    <span className="text-xs md:text-amber-400 text-amber-800">
                      {index + 1}
                    </span>
                  )}
                </div>
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap gap-1">
              {teamB.map((player, index) => (
                <div
                  key={index}
                  className={`w-6 h-6 rounded-full ${player.isEmpty ? "md:bg-white/30 md:border-dashed md:border-amber-500/30 bg-black/30 border-dashed border-amber-500/30" : "md:bg-white md:border-amber-500/30 bg-black border-amber-500/30"} flex items-center justify-center border`}
                >
                  {!player.isEmpty && (
                    <span className="text-xs md:text-gray-800 text-amber-400">
                      {index + 1}
                    </span>
                  )}
                </div>
          ...
          {queue.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {queue.map((player, index) => (
                <div
                  key={index}
                  className="flex items-center p-2 rounded-md bg-gray-800/50 border border-amber-500/10"
                >
                  ...
                  <div className="flex-1">
                    <div className="flex items-center justify-between">
                      <p
                        className={`font-medium ${player.isPlaceholder ? "text-white/50 italic" : "text-white"}`}
                      >
                        {player.user.name}
                      ...
                      {player.isPlaceholder ? (
                        ...
                        <>
                          ...
                          {player.user.skillLevel && (
                            <span>Skill: {player.user.skillLevel}/100</span>
                          )}
                        </>
                      )}
                    </div>
